...
## [v8.7.2] - 2026-08-21
### Performance Release: 아웃룩 파일 처리 속도 5초 ➔ 0.8초 초고속 최적화
- **세금계산서 복호화 고정 대기(5초) 제거 & 0.05초 동적 조건 감지**:
  기존의 `time.sleep(2)` + `time.sleep(3)` 강제 슬립을 완전히 제거하고, 비밀번호 입력란 및 iframe 렌더링을 0.05초 단위로 실시간 감지하여 즉시 다음 단계로 넘어가도록 개선 (복호화 및 PDF 변환 시간 4.5초 단축).
- **실시간 폴더 감시 반응 주기 대폭 단축 (`2000ms` ➔ `300ms`)**:
  `pdf_watcher` 타이머 간격을 0.3초로 단축하여 아웃룩 첨부파일 열람 또는 다운로드 시 0.3초 내 즉시 인식.
- **아웃룩 임시 폴더 3ms 초고속 스캔 최우선화**:
  `outlook_handler`에서 COM Dispatch 지연 없이 0.003초 만에 끝나는 Temp 폴더 및 클립보드를 1순위로 조회하여 즉시 파일 획득.
